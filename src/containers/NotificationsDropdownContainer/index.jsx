...
import { getNotifications, getCommunityNotifications, toggleNotificationSeen, markAllNotificationsRead, markAllNotificationsSeen, toggleNotificationRead,
  toggleBundledNotificationRead, viewOlderNotifications, hideOlderNotifications, resetNotifications, resetCommunityNotifications } from '../../actions/notifications'
import {
...
const NotificationsDropdownContainerView = (props) => {
  const {initialized, communityInitialized, isLoading, isCommunityLoading, sources, communitySources, notifications, communityNotifications, markAllNotificationsRead, toggleNotificationRead, toggleNotificationSeen,
  ...
  if ((!initialized && isLoading) || (!communityInitialized && isCommunityLoading)) {
    return (
  ...
  const hasUnread = notReadNotifications.length > 0 || notReadCommunityNotifications.length > 0
  ...
  if ((!isLoading && !initialized) || (!isCommunityLoading && !communityInitialized)) {
    notificationsEmpty = (
  ...
  const markNotificationsSeen = (isOpen) => {
    if (isOpen) {
      markAllNotificationsSeen(null, [...notifications, ...communityNotifications])
    }
  }
    ...
    <MediaQuery minWidth={SCREEN_BREAKPOINT_MD}>
      {(matches) => {
        if (matches) {
          ...
          const broadcastSource = communityNotificationsBySources.find(source => source.id === 'broadcast')
          const challengeSources = communityNotificationsBySources.filter(source => source.id !== 'broadcast')
                ...
                {!hasUnread ? (
                  ...
                  <ScrollLock key="body">
                    <div className="notifications-dropdown-body">
                      ...
                      <TransitionGroup>
                        {challengeSources.filter(source => source.notifications.length > 0).map(source => (
                          <Transition timeout={500} key={source.id} unmountOnExit>
                            {state => (
                              <NotificationsSection
                                {...source}
                      ...
                      <Transition in={broadcastSource && broadcastSource.notifications.length} timeout={500} unmountOnExit>
                        {state => (
                          <NotificationsSection
                            {...broadcastSource}
                            transitionState={state}
...
class NotificationsDropdownContainer extends React.Component {
  ...
  render() {
    ...
    const preRenderedCommunityNotifications = preRenderCommunityNotifications(communityNotifications)
    return (
